...
export const GET = withObservability({ route: "GET /api/verification/status" }, async (request: NextRequest, log) => {
  ...
  const isVerified = await Sentry.startSpan(
    { name: "verification.status.checkIsVerified", op: "db.near-contract", attributes: { account_id: accountId } },
  ...
  const redisStatus = await Sentry.startSpan(
    { name: "verification.status.getVerificationStatus", op: "db.redis", attributes: { account_id: accountId } },
  ...
  if (redisStatus) {
    ...
    if (!validatedResponse.success) {
      Sentry.captureMessage("verification_status_invalid_shape", "warning")
