...
export const POST = withObservability(
  ...
  async (request: NextRequest, log) => {
    ...
    try {
      ...
      if (externalUserId) {
        log.set("account_id", externalUserId)
      }
      ...
      if (payload.type === "applicantOnHold") {
        if (externalUserId) {
          await Sentry.startSpan(
            {
              name: "verification.webhook.setVerificationStatus",
              op: "db.redis",
      ...
      const applicant = await Sentry.startSpan(
        {
          name: "verification.webhook.getApplicant",
          op: "http.client",
      ...
      log.set("account_id", validAccountId)

      // Security: Verify externalUserId matches metadata to prevent request tampering
      if (externalUserId && externalUserId !== validAccountId) {
        await trackServerEvent(applicantId, {
          domain: "verification",
          action: "webhook_validation_fail",
      ...
      const keyCheck = await Sentry.startSpan(
        {
          name: "verification.webhook.hasFullAccessKey",
          op: "http.client",
      ...
      await Sentry.startSpan(
        {
          name: "verification.webhook.ensureRedisInitialized",
          op: "db.redis",
      ...
      try {
        await Sentry.startSpan(
          {
            name: "verification.webhook.storeVerification",
            op: "http.client",
        ...
        await Sentry.startSpan(
          {
            name: "verification.webhook.clearVerificationStatus",
            op: "db.redis",
            attributes: { account_id: validAccountId },
          },
        ...
        log.setAll({
          webhook_outcome: "onchain_store_failed",
          error_code: errorCodeForLog,
...
export const GET = withObservability(
  { route: "GET /api/verification/sumsub/webhook" },
