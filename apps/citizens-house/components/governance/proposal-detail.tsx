...
export function ProposalDetail({ proposal, isAdmin }: Props) {
  const [now, setNow] = useState(Date.now)
  const isScheduled = proposal.status === "active" && proposal.startAt > now
  const displayStatus = isScheduled ? "scheduled" : proposal.status
  ...
  const handleAdminAction = async (action: "cancel" | "expire" | "finalize") => {
    if (!isConnected || !accountId) return

    setTxLoading(true)
    try {
      const txBuilders = {
        cancel: () => buildCancelProposalTx(proposal.id),
        expire: () => buildExpirePendingProposalTx(proposal.id),
        finalize: () => buildFinalizeProposalTx(proposal.id),
      }
      await signAndSendTransaction(txBuilders[action]())
      trackEvent({
        domain: "governance",
        action: action === "finalize" ? "proposal_finalize" : "proposal_cancel",
      ...
      toast.success(
        `Proposal ${action === "finalize" ? "finalized" : action === "cancel" ? "cancelled" : "expired"} successfully`,
      )
    } catch (error) {
      ...
      {isAdmin && (
        <div className="bg-white dark:bg-[#191a23] border border-[rgba(0,0,0,0.1)] dark:border-white/20 rounded-[16px] p-6">
          ...
          <div className="flex flex-wrap gap-3">
            ...
            {proposal.status === "active" && Date.now() > proposal.endsAt && (
              <Button
                variant="citizens-primary"
                size="citizens-lg"
                onClick={() => handleAdminAction("finalize")}
                disabled={loading}
              >
                {loading ? <Loader2 className="h-4 w-4 animate-spin" /> : null}
                Finalize
