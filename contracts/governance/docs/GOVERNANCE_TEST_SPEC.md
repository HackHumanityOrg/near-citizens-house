...
4. Call `new` with `quorum_bps = 10_001`.
   Expected results:
5. Steps 1-2 succeed.
6. Steps 3-4 panic with `ERR_QUORUM_BPS_OUT_OF_RANGE`.
...
4. Call `new` with `voting_period_secs = MAX_VOTING_PERIOD_SECS + 1`.
   Expected results:
5. Steps 1-2 succeed.
6. Steps 3-4 panic with `ERR_VOTING_PERIOD_OUT_OF_RANGE`.
...
4. Call `new` with `pending_expiry_secs` above max.
   Expected results:
5. Steps 1-2 succeed.
6. Steps 3-4 panic with `ERR_PENDING_EXPIRY_OUT_OF_RANGE`.
...
4. Call `new` with `min_proposal_bond > 100 NEAR`.
   Expected results:
5. Steps 1-2 succeed.
6. Steps 3-4 panic with `ERR_MIN_BOND_OUT_OF_RANGE`.
...
4. Call `new` above max.
   Expected results:
5. Steps 1-2 succeed.
6. Steps 3-4 panic with `ERR_GRACE_PERIOD_OUT_OF_RANGE`.
...
3. Call `new` with `max_start_delay_secs > MAX_START_DELAY_SECS`.
   Expected results:
4. Steps 1-2 succeed.
5. Step 3 panics with `ERR_MAX_START_DELAY_OUT_OF_RANGE`.
...
4. Call `list_admins(limit=101)`.
   Expected results:
5. Step 1 returns true.
6. Steps 2-3 return expected paginated results.
7. Step 4 panics with `ERR_LIMIT_TOO_LARGE`.
...
3. Call `create_proposal` with empty description.
   Expected results:
4. Panics with `ERR_TITLE_EMPTY`, `ERR_AUTHOR_EMPTY`, `ERR_DESCRIPTION_EMPTY` respectively.
...
6. Call with description len 10_001.
   Expected results:
7. Steps 1,3,5 succeed.
8. Steps 2,4,6 panic with `ERR_TITLE_TOO_LONG`, `ERR_AUTHOR_TOO_LONG`, `ERR_DESCRIPTION_TOO_LONG`.
...
2. Call `create_proposal` with attached deposit = MIN_BOND.
   Expected results:
3. Step 1 panics with `ERR_INSUFFICIENT_BOND`.
4. Step 2 succeeds, proposal stored as Pending.
...
4. Call with `start_at = T + max_start_delay_secs * 1e9 + 1`.
   Expected results:
5. Step 1 succeeds.
6. Step 2 panics with `ERR_START_AT_BEFORE_CREATED`.
7. Step 3 succeeds.
8. Step 4 panics with `ERR_START_AT_TOO_FAR`.
...
2. Call `create_proposal` with a title of 141 bytes using multi-byte UTF-8 characters.
   Expected results:
3. Step 1 succeeds.
4. Step 2 panics with `ERR_TITLE_TOO_LONG`.
...
5. Set status to `Active`, call `cancel_proposal`.
   Expected results:
6. Steps 1-2 panic `ERR_PROPOSAL_ALREADY_FINALIZED`.
7. Step 3 panics `ERR_PROPOSAL_ALREADY_CANCELLED`.
8. Steps 4-5 succeed, status becomes `Cancelled`, event emitted.
...
2. Set `block_timestamp = T`, call `expire_pending_proposal`.
   Expected results:
3. Step 1 panics `ERR_PROPOSAL_NOT_EXPIRED`.
4. Step 2 succeeds, status `Failed`, failure_kind `PendingExpired`, event emitted.
...
4. Set status to Failed, call `expire_pending_proposal`.
   Expected results:
5. Steps 1-4 panic with `ERR_PROPOSAL_NOT_PENDING`.
...
3. Call finalize on Succeeded or Failed.
   Expected results:
4. All steps panic with `ERR_PROPOSAL_NOT_ACTIVE`.
...
2. Set `block_timestamp = T + 1`, call finalize.
   Expected results:
3. Step 1 panics `ERR_FINALIZE_NOT_ENDED`.
4. Step 2 proceeds to quorum checks and status update.
...
2. Set `block_timestamp = T + G`, call finalize.
   Expected results:
3. Step 1 panics `ERR_FINALIZE_BLOCKED_BY_PENDING_VOTES`.
4. Step 2 proceeds (pending votes treated as abandoned).
...
2. Set yes/no votes totals to below, equal, and above quorum.
   Expected results:
3. Below quorum -> Failed with `QuorumNotMet`.
4. At or above quorum and yes >= no -> Succeeded.
5. At or above quorum and yes < no -> Failed with `Rejected`.
...
2. Finalize proposal.
   Expected results:
3. Proposal Succeeded (ties pass).
...
2. Check pending_votes entries for the voters.
   Expected results:
3. Pending vote entries remain unchanged.
4. pending_vote_count is unchanged by cancel.
...
2. Set block_timestamp = T and call expire_pending_proposal.
   Expected results:
3. Step 1 panics ERR_PROPOSAL_NOT_EXPIRED.
4. Step 2 succeeds (does not require ends_at).
...
1. Create three proposals.
   Expected results:
1. IDs are 0, 1, 2.
1. get_proposal_count returns 3.
...
1. ends_at == created_at + voting_period_secs \* 1e9.
...
1. pending_vote_cleared event deposit_refunded matches.
...
1. pending_vote_cleared event deposit_refunded is 0.
...
2. Call `get_vote(proposal_id, voter)`.
   Expected results:
3. Step 1 returns false.
4. Step 2 returns None.
...
2. Call `get_vote(proposal_id, voter)`.
   Expected results:
3. Step 1 returns true.
4. Step 2 returns VoteView with correct proposal_id, voter, choice, and voted_at.
...
3. Call `list_blocklist(from_index=0, limit=101)`.
   Expected results:
4. Step 1 returns 1 item.
5. Step 2 returns empty list.
6. Step 3 panics with `ERR_LIMIT_TOO_LARGE`.
...
4. `block_timestamp = T2 + 1`, cast.
   Expected results:
5. Step 1 panics `ERR_PROPOSAL_NOT_STARTED`.
6. Steps 2-3 proceed to pending vote creation.
7. Step 4 panics `ERR_PROPOSAL_ENDED`.
...
2. Call `cast_vote` with deposit >= `storage_cost`.
   Expected results:
3. Step 1 panics with `ERR_INSUFFICIENT_DEPOSIT`.
4. Step 2 succeeds and records PendingVote.
...
2. Voter casts vote on proposal B.
   Expected results:
3. Both pending votes are recorded independently.
...
1. `pending_blocklist_op_cleared` event emitted.
...
2. Call below min, above max.
   Expected results:
3. Step 1 succeeds.
4. Step 2 panics `ERR_QUORUM_BPS_OUT_OF_RANGE`.
...
2. Read existing proposal timing before and after update.
   Expected results:
3. Update succeeds.
4. Existing proposal `ends_at` remains unchanged.
...
2. Call below min and above max.
   Expected results:
3. Step 1 succeeds.
4. Step 2 panics `ERR_VOTING_PERIOD_OUT_OF_RANGE`.
...
2. Call below min, above max.
   Expected results:
3. Step 1 succeeds.
4. Step 2 panics `ERR_PENDING_EXPIRY_OUT_OF_RANGE`.
...
2. Call below MIN_BOND and above max.
   Expected results:
3. Step 1 succeeds.
4. Step 2 panics `ERR_MIN_BOND_OUT_OF_RANGE`.
...
2. Call below min and above max.
   Expected results:
3. Step 1 succeeds.
4. Step 2 panics `ERR_GRACE_PERIOD_OUT_OF_RANGE`.
...
2. Call above max.
   Expected results:
3. Step 1 succeeds.
4. Step 2 panics `ERR_MAX_START_DELAY_OUT_OF_RANGE`.
...
1. quorum_bps and duration fields are JSON numbers.
...
1. `proposal_creation_failed` event emitted.
...
2. Call with verified_count = N, blocklist size = N.
   Expected results:
3. Proposal status Failed, failure_kind `ZeroSnapshot`.
4. `proposal_creation_failed` event emitted.
...
1. `proposal_activated` event emitted.
...
1. `vote_rejected` emitted with `CallbackFailed`.
1. Deposit refunded if present.
...
3. Set proposal status Failed.
   Expected results:
4. `vote_rejected` with `ProposalCancelled`.
5. `vote_rejected` with `PostFinalize`.
6. `vote_rejected` with `PostFinalize`.
7. PendingVote removed, deposit refunded.
...
1. Returns false, no event emitted.
...
2. Return `Ok(None)`.
   Expected results:
3. `vote_rejected` with `CallbackFailed` and refund.
4. `vote_rejected` with `NotVerified` and refund.
...
2. Set submitted_at > ends_at.
   Expected results:
3. `vote_rejected` with `ProposalExpired` and refund.
...
1. Call callback with valid verification.
   Expected results:
1. PendingVote removed, pending_vote_count decremented.
1. Vote recorded with `voted_at = submitted_at`.
1. yes/no tallies updated.
1. Refund of excess deposit computed from storage delta.
1. `vote_cast` event emitted.
...
2. Call callback with valid verification.
   Expected results:
3. get_vote returns voted_at == submitted_at.
...
1. PendingVote removed, deposit refunded.
1. Returns false, no vote recorded.
...
2. Return `Ok(None)`.
   Expected results:
3. pending op cleared, blocklist unchanged.
...
1. Return `Ok(Some(VerificationSummary))`.
   Expected results:
1. pending op cleared.
1. account added to blocklist, `blocklist_added` event emitted.
...
1. Return `Ok(Some(VerificationSummary))`.
   Expected results:
1. pending op cleared.
1. blocklist unchanged, no `blocklist_added` event.
...
7. Call finalize.
   Expected results:
8. Proposal transitions Pending -> Active -> Succeeded.
9. Yes/No tallies match votes, quorum satisfied.
10. Events emitted: proposal_created, proposal_activated, vote_cast x4, proposal_finalized.
...
3. Fast-forward to after ends_at and finalize.
   Expected results:
4. Proposal finalizes Failed with `Rejected`.
...
3. Finalize after ends_at.
   Expected results:
4. Proposal finalizes Failed with `QuorumNotMet`.
...
3. Fast-forward to start_at and vote.
   Expected results:
4. Vote before start_at rejected with `ERR_PROPOSAL_NOT_STARTED`.
5. Vote at/after start_at accepted.
...
2. Finalize each proposal after its ends_at.
   Expected results:
3. Tallies and status are independent per proposal.
...
2. Create another proposal.
   Expected results:
3. Second proposal has next ID and functions normally.
...
3. Fast-forward to ends_at + 1 and finalize.
   Expected results:
4. Behavior matches start_at/ends_at window.
...
2. Allow callbacks to resolve.
   Expected results:
3. All pending votes coexist and final tallies are correct.
...
2. Let callback return error.
   Expected results:
3. Proposal status Failed with `SnapshotCallbackFailed`.
4. `proposal_creation_failed` event emitted.
...
2. Snapshot returns 0 or effective count 0.
   Expected results:
3. Proposal status Failed with `ZeroSnapshot`.
4. `proposal_creation_failed` emitted.
...
3. Admin calls `expire_pending_proposal`.
   Expected results:
4. Proposal status Failed with `PendingExpired`.
5. `pending_proposal_expired` event emitted.
...
2. Delay callback execution until after `ends_at`.
   Expected results:
3. Vote accepted since submitted_at within window.
4. Tallies reflect vote.
...
3. Allow callback to execute after finalization.
   Expected results:
4. Vote rejected with `PostFinalize`.
5. PendingVote removed and deposit refunded.
...
2. Observe callback failure.
   Expected results:
3. `vote_rejected` with `CallbackFailed`.
...
2. Admin calls `clear_stale_pending_vote`.
   Expected results:
3. PendingVote removed, pending_vote_count decremented.
4. Deposit refunded to voter.
5. `pending_vote_cleared` event emitted.
...
3. Finalize proposal.
   Expected results:
4. Finalize succeeds after pending vote cleared.
...
2. Call finalize.
   Expected results:
3. Reverts with `ERR_FINALIZE_BLOCKED_BY_PENDING_VOTES`.
...
2. Call finalize.
   Expected results:
3. Finalize succeeds, pending votes treated as abandoned.
...
2. Mock returns verification for v1.
   Expected results:
3. v1 added to blocklist, event emitted.
...
2. Mock returns None.
   Expected results:
3. Blocklist not changed, pending op cleared.
...
2. Admin calls `blocklist_account(v2)`.
   Expected results:
3. Step 2 reverts with `ERR_BLOCKLIST_OP_PENDING`.
...
5. Resolve blocklist callback.
   Expected results:
6. Returns false when idle.
7. Returns true during Pending/Active.
8. Returns false after finalize.
9. Returns true while pending op.
10. Returns false after callback.
...
3. Create a new proposal after the update.
   Expected results:
4. Call succeeds.
5. Existing active proposal keeps its original `ends_at`.
6. New proposal uses the updated voting period.
...
2. Create proposal and verify snapshot uses new contract (by mocking different verified count).
   Expected results:
3. `config_updated` event emitted.
4. Snapshot reflects new contract result.
...
2. Update quorum_bps to 5000, create proposal B.
   Expected results:
3. Proposal A has quorum_bps = 700.
4. Proposal B has quorum_bps = 5000.
...
2. Attempt finalize at new grace deadline.
   Expected results:
3. Finalize respects the updated grace period value.
...
2. Let callback succeed.
   Expected results:
3. Vote recorded.
4. Refund equals `deposit - actual_storage_cost`.
...
3. Capture storage usage after callback.
   Expected results:
4. Refund equals `deposit - (storage_after - storage_before) * storage_byte_cost`.
   Notes:
5. This test is optional if storage deltas are unstable across SDK versions.
...
1. Full 5 NEAR retained by contract.
...
2. Create second proposal.
   Expected results:
3. First call returns 0, second returns 1.
...
3. Admin calls `migrate` with 1 yocto.
   Expected results:
4. Step 1 panics `ERR_NOT_ADMIN`.
5. Step 2 panics assert-one-yocto.
6. Step 3 succeeds and preserves state.
...
- Requirements on proposal lifecycle, voting, callbacks, blocklist, and config are covered by UT-PROP-_, UT-VOTE-_, UT-SNAP-_, UT-BLOCK-_, UT-CONFIG-_, and IT-E2E-_ cases.
- Storage model and refunds are covered by UT-VOTE-006, UT-VOTE-CB-007, IT-STORE-\* cases.
- Edge cases and boundaries are covered by UT-INIT-_, UT-PROP-_, UT-VOTE-002, and UT-CONFIG-\* cases.
