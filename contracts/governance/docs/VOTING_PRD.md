...
- **Passing condition**: Quorum met and `yes_votes >= no_votes` (ties pass). Quorum is met when `(yes_votes + no_votes) >= quorum_required`.
...
- **is_vote_free()** view — Returns `true` if contract has enough balance to cover vote storage, `false` if deposit is required. Useful for frontend UX; treat as a balance-only hint (may change before the vote is recorded).
...
- **Stuck blocklist op**: A failed blocklist callback could leave a pending op that blocks further blocklist changes and proposal creation. Mitigation: admin-only `clear_stale_blocklist_op` recovery method and event logging.
...
- **Callback gas**: Snapshot callback uses at least 25 Tgas; vote callback uses at least 45 Tgas; blocklist callback uses at least 25 Tgas.
