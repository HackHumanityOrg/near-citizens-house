...
#[allure_test]
#[test]
fn test_verification_summary_json_missing_sumsub_applicant_id_fails() {
    // JSON without required "sumsub_applicant_id" field should fail deserialization
    ...
    assert!(
        result.is_err(),
        "Deserialization should fail when sumsub_applicant_id is missing"
    );
}
...
fn test_verification_summary_json_missing_near_account_id_fails() {
    ...
    let json = r#"{"sumsub_applicant_id":"sumsub123","verified_at":0}"#;
    let result: Result<VerificationSummary, _> = near_sdk::serde_json::from_str(json);
    assert!(
        result.is_err(),
...
fn test_verification_json_missing_user_context_data_fails() {
    ...
    let json =
        r#"{"sumsub_applicant_id":"sumsub123","near_account_id":"test.near","verified_at":0}"#;
...
fn test_verification_summary_json_invalid_timestamp_type_fails() {
    ...
    let json = r#"{"sumsub_applicant_id":"sumsub123","near_account_id":"test.near","verified_at":"not_a_number"}"#;
    let result: Result<VerificationSummary, _> = near_sdk::serde_json::from_str(json);
    assert!(
        result.is_err(),
