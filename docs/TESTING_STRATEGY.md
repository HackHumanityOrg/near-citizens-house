...
- SputnikDAO proposal payload formatting (Vote, AddMemberToRole, ChangePolicyAddOrUpdateRole)
- Description validation (max 10,000 chars, non-empty; bridge-enforced limit)
- Quorum calculation (7% of citizen count with ceiling division)
- Authorization checks (backend wallet only for all write operations)
...
**sputnik-bridge integration:**

_Setup & Configuration:_

- Contract initialization with all parameters
- Bridge connected to DAO with correct permissions
- DAO policy configured with citizen and bridge roles
- Bridge info getter returns correct values

_Member Addition:_

- Add verified member succeeds
- Add unverified member fails
- Add member creates proposal in DAO
- Add member auto-approves proposal
- Member appears in citizen role after addition
- Add member unauthorized (non-backend) fails
- Add multiple members sequentially
- Add same member twice (idempotent behavior)

_Proposal Creation:_

- Create vote proposal succeeds (backend wallet only)
- Create proposal returns incrementing ID
- Create proposal unauthorized fails
- Empty description rejected
- Whitespace-only description rejected
- Description at max length (10,000 chars) succeeds
- Description over max length fails
- Unicode in description preserved
- Insufficient deposit fails

_Voting:_

- Citizen can vote on proposal
- Vote approve passes proposal
- Vote reject fails proposal
- Non-citizen cannot vote
- Proposal passes with majority yes
- Proposal fails with majority no
- Double vote on same proposal fails
- Vote on nonexistent proposal fails
- Vote before expiry succeeds
- Vote after expiry fails
- Vote at exact expiry boundary (edge case)
- Proposal finalization after voting period

_Dynamic Quorum:_

- Quorum calculation (7% with ceiling)
- Quorum updates when citizens added
- Quorum with 1 citizen = 1
- Quorum with 15 citizens = 2
- Quorum with 50+ citizens scales correctly
- Proposal fails when quorum not met
- Proposal passes when quorum met but threshold not
...
- Citizens cannot add proposals to DAO directly
- Random accounts cannot add proposals to DAO
- Citizens cannot vote remove (only approve/reject)
- Anyone can finalize expired proposals
- Callback functions cannot be called externally (#[private])

_Failure Handling:_

- Verification check fails → no DAO proposal created
- Auto-approve fails → no member_added event
- Verification promise fails → no event emitted
- DAO proposal creation fails → no event emitted
- Multiple failures don't corrupt state
- State consistency after partial callback chain failure

_Gas & Performance:_

- Callback chain completes within 300 TGas limit
- Gas exhaustion mid-callback-chain (graceful failure)
- Insufficient gas for verification cross-contract call

_Events:_

- member_added event with correct data (member_id, role, proposal_id)
- proposal_created event with correct data (proposal_id, description)
- quorum_updated event with correct data (citizen_count, new_quorum, proposal_id)

_Concurrency:_

- Parallel add_member calls for same user (race condition)
- Backend wallet rotation during pending operation
- Pause contract during pending verification

_Scale:_

- 50+ citizens with quorum calculations
- 100+ proposals in DAO
